  ...
  useEffect(() => {
    const initDocs = async () => {
      ...
      if (docsTree.length === 0) {
        ...
        // Expand root folders
        const roots = new Set(
          docsTree.filter((n) => n.type === "folder").map((n) => n.id)
        );
            ...
            <ReactMarkdown
              components={{
                code({ node, inline, className, children, ...props }: any) {
                  ...
                  return (
                    <code
                      className="bg-white/10 rounded px-1.5 py-0.5 text-origami-cyan font-mono text-sm mx-0.5"
                      {...props}
                    >
                      {children}
                    </code>
                ...
                a({ node, href, children, ...props }: any) {
                      ...
                      <button
                        onClick={(e) => {
                          ...
                          if (activeDoc) {
                            ...
                            const baseDir = activeDoc.split("/").slice(0, -1);
                            ...
                            const targetPath = baseDir.join("/");
                            loadContent(targetPath);
                          }
                        }}
                        className="text-origami-cyan hover:underline bg-transparent border-none p-0 cursor-pointer inline font-medium"
                        {...props}
                      >
                        {children}
                      </button>
                  ...
                  return (
                    <a
                      ...
                      className="text-origami-cyan hover:underline"
                      {...props}
                    >
                      {children}
                    </a>
